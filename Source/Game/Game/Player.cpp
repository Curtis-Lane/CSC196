...
void Player::Update(float deltaTime) {
	...
	if(ane::globalInputSystem.GetKeyDown(SDL_SCANCODE_SPACE) && !ane::globalInputSystem.GetPreviousKeyDown(SDL_SCANCODE_SPACE)) {
		...
		if(it != this->powerups.end()) {
			...
			std::unique_ptr<Rocket> rocket = std::make_unique<Rocket>(400.0f, rocketTransform1, this->model);
			rocket->tag = "Player";
			...
			rocket = std::make_unique<Rocket>(400.0f, rocketTransform2, this->model);
			rocket->tag = "Player";
			this->scene->Add(std::move(rocket));
		} else {
	...
	if(ane::globalInputSystem.GetKeyDown(SDL_SCANCODE_T)) {
